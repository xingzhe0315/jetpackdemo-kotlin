...
abstract class BaseListViewModel<Data> : BaseViewModel<List<Data>>() {
        ...
        getLoadMoreObservable().subscribeOn(Schedulers.io())
            ...
            .subscribe(object : BaseObserver<BaseResponseData<*>>() {
                override fun onDataSuccess(responses: List<BaseResponseData<*>>) {
                    ...
                    appendData(
                        if (isSingle())
                            getSingleDataFromResponse(responses[0])
                        else
                            getMultiDataFromResponses(responses)
                    )
                }
    ...
    override fun getDataFromObserver(responses: List<BaseResponseData<*>>) {
        super.getDataFromObserver(responses)
        headerDataList.value = getHeaderDataFromResponses(responses)
    }

    override fun getSingleDataFromResponse(response: BaseResponseData<*>): List<Data>? {
        return getListDataFromResponse(response)
    }

    override fun getMultiDataFromResponses(responses: List<BaseResponseData<*>>): List<Data>? {
